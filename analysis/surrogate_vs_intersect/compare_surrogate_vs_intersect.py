...
CASE_RE = re.compile(r"_(?P<geo>\d+)_run(?P<run>\d+)$")
...
def parse_case_name(case_name: str) -> tuple[int, int] | None:
...
def load_manifest_predictions(manifest_path: Path, repo_root: Path) -> tuple[list[dict[str, Any]], dict[int, list[dict[str, Any]]], dict[int, dict[str, Any]]]:
    ...
    run_to_wells: dict[int, list[dict[str, Any]]] = {}
    ...
    for snap in manifest.get("snapshots", []):
        ...
        run_to_wells[run_id] = payload.get("wells", [])
...
def load_intersect_actuals(h5_path: Path) -> dict[tuple[int, int], dict[str, float]]:
...
def save_runid_trend_plot(
    ...
    for pred_key, act_key, label, ax in specs:
        ...
        all_run_ids = sorted({int(r["run_id"]) for r in rows})
        surrogate_avg = []
        actual_avg = []

        for g in geos:
            geo_rows = sorted(
                [r for r in rows if int(r["geology_config_id"]) == g],
                key=lambda row: int(row["run_id"]),
            )
            run_ids = [int(r["run_id"]) for r in geo_rows]
            pred_vals = [float(r[pred_key]) for r in geo_rows]
            act_vals = [float(r[act_key]) for r in geo_rows]
            geo_name = geology_info.get(g, {}).get("geology_name", f"geo_{g}")
            ...
            ax.plot(
                ...
                linewidth=1.2,
                linestyle="-",
                label=f"{geo_name} surrogate" if label == "Total Energy" else None,
            )
            ax.plot(
                run_ids,
                act_vals,
                ...
                linewidth=1.2,
                linestyle="--",
                label=f"{geo_name} intersect" if label == "Total Energy" else None,
            )

        for run_id in all_run_ids:
            run_rows = [r for r in rows if int(r["run_id"]) == run_id]
            if not run_rows:
                continue
            surrogate_avg.append(float(np.mean([float(r[pred_key]) for r in run_rows])))
            actual_avg.append(float(np.mean([float(r[act_key]) for r in run_rows])))

        # Scatter the per-run averages and fit linear trendlines
        x = np.array(all_run_ids[: len(surrogate_avg)])
        y_s = np.array(surrogate_avg)
        y_a = np.array(actual_avg)

        ax.scatter(x, y_s, color=MANIM_WHITE, alpha=0.9, s=30, label="Avg surrogate (per run)")
        ax.scatter(x, y_a, color=MANIM_ORANGE, alpha=0.9, s=30, label="Avg intersect (per run)")

        if len(x) >= 2:
            # fit linear trendlines
            coeffs_s = np.polyfit(x, y_s, deg=1)
            coeffs_a = np.polyfit(x, y_a, deg=1)
            xs = np.linspace(x.min(), x.max(), 200)
            ys = np.polyval(coeffs_s, xs)
            ya = np.polyval(coeffs_a, xs)
            ax.plot(xs, ys, color=MANIM_WHITE, linewidth=2.5, linestyle="-", label="Trend surrogate")
            ax.plot(xs, ya, color=MANIM_ORANGE, linewidth=2.5, linestyle="--", label="Trend intersect")

        ax.set_ylabel(label)
        ax.set_title(f"{label}: Surrogate vs Intersect by Run ID")
        ax.grid(True, linestyle="--", alpha=0.2, color=MANIM_GREY)
    ...
    for pred_key, actual_key, slug, title in metric_specs:
        ...
        for ridx, geo in enumerate(geos):
            ...
            for cidx, (label, row) in enumerate((("Best", best), ("Worst", worst))):
                ax = axes[ridx, cidx]
                wells = run_to_wells.get(int(row["run_id"]), [])
                ...
                ax.set_title(
                    (
                        ...
                        f"run={int(row['run_id']):04d}, pred={pred_val:.3e}, "
...
def main() -> None:
    ...
    missing_actual = 0
    for p in pred_rows:
        key = (int(p["geology_config_id"]), int(p["run_id"]))
        act = actuals.get(key)
        if act is None:
            missing_actual += 1
            continue
    ...
    save_runid_trend_plot(
        matched, geology_info, output_dir / "runid_surrogate_vs_intersect_trends.png"
    )
    ...
    print(f"  - {output_dir / 'runid_surrogate_vs_intersect_trends.png'}")
