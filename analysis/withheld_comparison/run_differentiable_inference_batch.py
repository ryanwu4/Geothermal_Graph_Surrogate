...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Well Placement Differentiable Optimization (LHS Batch Geologies)"
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    if not geology_files or geology_files[0] is None:
        raise ValueError("Missing 'geology_h5_files' or 'geology_h5_file' in configuration")
    ...
    if checkpoint_path.exists():
        model = HeteroGNNRegressor.load_from_checkpoint(str(checkpoint_path), map_location=device)
    ...
    for src in open_h5_handles:
        ...
        for prop in PROPERTIES:
            ...
            normalized = (data - p_min) / (p_max - p_min) if p_max > p_min else np.zeros_like(data)
    ...
    # Plot base arrays
    perm_x_grids = [p["physics_dict"]["PermX"].cpu().numpy() for p in h5_physics_contexts]
    z_slice = perm_x_grids[0].shape[0] // 2
    backgrounds = [grid[z_slice, :, :].T for grid in perm_x_grids]
    
    # 3. Generate N Initial Layout Samples via Latin Hypercube
    num_wells = len(config["wells"])
    print(f"Generating {num_samples_N} LHS starting configurations for {num_wells} wells...")
    
    sampler = qmc.LatinHypercube(d=num_wells * 2, seed=42)
    ...
    for i in range(num_samples_N):
        ...
        for w in range(num_wells):
            rand_x = x_min_bound + lhs_samples[i, w*2] * (x_max_bound - x_min_bound)
            rand_y = y_min_bound + lhs_samples[i, w*2 + 1] * (y_max_bound - y_min_bound)
    ...
    print(f"\n--- Starting Batched Well Placement Optimization ---")
    for chunk_start in range(0, num_samples_N, batch_size_M):
        batch_start_time = time.time()
        chunk_cfgs = lhs_configs[chunk_start:chunk_start+batch_size_M]
        M_actual = len(chunk_cfgs)
        chunk_graphs = []
        
        print(f"Loading/Building PyG models for batch {chunk_start // batch_size_M + 1} ({M_actual} configurations evaluated structurally over {K} geologies)...")
        # Build batched graphs identically
        for m_idx, w_cfg in enumerate(chunk_cfgs):
            for k_idx, (src, p_ctx) in enumerate(zip(open_h5_handles, h5_physics_contexts)):
                ...
                (x_idx, y_idx, depth, inj, perm_x, perm_y, perm_z, porosity, temp0, press0, depth_centroid) = extract_well_data(is_well, inj_rate, src)
                wells = build_wells_table(x_idx, y_idx, depth, inj, perm_x, perm_y, perm_z, porosity, temp0, press0)
                vertical_profiles = extract_vertical_profiles(is_well, x_idx, y_idx, src)

                raw_graph = build_single_hetero_data(
                    wells=wells, physics_dict=p_ctx["physics_dict"], full_shape=p_ctx["full_shape"],
                    target="graph_energy_total", target_val=0.0, vertical_profile=vertical_profiles, case_id=f"run{chunk_start+m_idx}_geo{k_idx}"
                )
        ...
        for w_cfg in chunk_cfgs:
            c = []
            for w in w_cfg: c.append([w["x"], w["y"], float(w["depth"])])
            continuous_starts.append(c)
        
        base_coords = torch.tensor(continuous_starts, dtype=torch.float32, device=device) # (M, W, 3)
        base_coords.requires_grad = True
        
        optimizer = optim.Adam([base_coords], lr=args.learning_rate)
        
        chunk_coords_hist = [base_coords.clone().detach().cpu().numpy()] # list of (M, W, 3)
        ...
        print(f"Computed batch {chunk_start // batch_size_M + 1} / {int(np.ceil(num_samples_N / batch_size_M))} in {batch_elapsed:.2f}s")
    ...
    for k in range(K):
        ...
        bg = backgrounds[k]
        im = ax_map.imshow(
            bg, origin="lower", cmap="viridis", alpha=0.55,
        ...
        ax_map.set_ylabel("Y Coordinate" if k == 0 else "", fontsize=FONT_SIZE, color=MANIM_WHITE)
        geo_name = Path(geology_files[k]).stem
        ax_map.set_title( f"{geo_name}\nFinal Positions Only (Z={z_slice})", fontsize=TITLE_SIZE, color=MANIM_WHITE)

    steps_range = range(args.optimization_steps + 1)
    for n_idx in range(num_samples_N):
        mean_track = all_energy_tracks[n_idx].mean(axis=1)
        ax_energy1.plot(
            steps_range, mean_track,
            linestyle="-", alpha=0.25, linewidth=2.0, color=MANIM_BLUE,
        )

    ax_energy1.plot([], [], linestyle="-", alpha=0.5, linewidth=2.0, color=MANIM_BLUE, label="Mean Batch Energy")
    ax_energy1.set_xlabel("Optimization Iteration", fontsize=FONT_SIZE, color=MANIM_WHITE)
    ax_energy1.set_ylabel("Total Energy (Non-Norm)", fontsize=FONT_SIZE, color=MANIM_WHITE)
    ax_energy1.set_title(f"Energy Traces across {num_samples_N} LHS Samples", fontsize=TITLE_SIZE, color=MANIM_WHITE)
    ax_energy1.grid(True, linestyle="--", alpha=0.3, color=MANIM_GREY)
    ax_energy1.legend(fontsize=LEGEND_SIZE, facecolor="#111111", edgecolor=MANIM_GREY, labelcolor=MANIM_WHITE)

    plt.tight_layout()
    plot_path1 = f"plots/well_trajectories_batch_{timestamp}_all.png"
    ...
    for k in range(K):
        ...
        labels_done = False
        colors = {"Best": "#00FF00", "Avg": "#58C4DD", "Worst": "#FF0000"}  # Green, Blue, Red
        ...
        for trace_name, idx in subsets:
            ...
            for w in range(num_wells):
                ...
                ax_map.plot(
                    history[:, w, 0], history[:, w, 1],
                    color=trace_color, alpha=0.9, linestyle="-", linewidth=lw,
                    label=trace_name if (w == 0 and not labels_done and k == 0) else None,
                )
                
                # Starting circle
                ax_map.scatter(
                    history[0, w, 0], history[0, w, 1],
                    color=trace_color, marker="o", s=80,
                ...
                ax_map.scatter(
                    history[-1, w, 0], history[-1, w, 1],
                    color=trace_color, marker=marker_end, s=200,
                    edgecolors=MANIM_WHITE, linewidths=1.2, zorder=6 + (2 if trace_name == "Best" else 0),
                )
            labels_done = True
        ...
        ax_map.set_ylabel("Y Coordinate" if k == 0 else "", fontsize=FONT_SIZE, color=MANIM_WHITE)
        geo_name = Path(geology_files[k]).stem
        ax_map.set_title( f"{geo_name}\nBest/Avg/Worst PermX (Z={z_slice})", fontsize=TITLE_SIZE, color=MANIM_WHITE)
        if k == 0:
            ax_map.legend(fontsize=LEGEND_SIZE, facecolor="#111111", edgecolor=MANIM_GREY, labelcolor=MANIM_WHITE)
    ...
    for trace_name, idx in subsets:
        ...
        ax_energy2.plot(
            steps_range, mean_track,
            linestyle="-", alpha=0.9, linewidth=linewidths[trace_name], color=colors[trace_name], label=trace_name
        )

    ax_energy2.set_xlabel("Optimization Iteration", fontsize=FONT_SIZE, color=MANIM_WHITE)
    ax_energy2.set_ylabel("Total Energy (Non-Norm)", fontsize=FONT_SIZE, color=MANIM_WHITE)
    ax_energy2.set_title("Energy Traces (Highlights)", fontsize=TITLE_SIZE, color=MANIM_WHITE)
    ax_energy2.grid(True, linestyle="--", alpha=0.3, color=MANIM_GREY)
    ax_energy2.legend(fontsize=LEGEND_SIZE, facecolor="#111111", edgecolor=MANIM_GREY, labelcolor=MANIM_WHITE)

    plt.tight_layout()
    plot_path2 = f"plots/well_trajectories_batch_{timestamp}_highlights.png"
    ...
    # Injector Heatmap
    hb_inj = axes3[0].hexbin(final_inj_x, final_inj_y, gridsize=20, cmap="Blues", extent=[0, nx, 0, ny], mincnt=1, alpha=0.95, edgecolor="none")
    ...
    axes3[0].set_title(f"Injector Final Placements", fontsize=TITLE_SIZE, color=MANIM_WHITE)

    # Producer Heatmap
    hb_prd = axes3[1].hexbin(final_prd_x, final_prd_y, gridsize=20, cmap="Oranges", extent=[0, nx, 0, ny], mincnt=1, alpha=0.95, edgecolor="none")
    ...
    axes3[1].set_title(f"Producer Final Placements", fontsize=TITLE_SIZE, color=MANIM_WHITE)
    ...
    plt.tight_layout()
    plot_path3 = f"plots/well_trajectories_batch_{timestamp}_heatmap.png"
    fig3.savefig(plot_path3, dpi=200, bbox_inches="tight", facecolor=MANIM_BG)
    plt.close(fig3)

    print(f"Saved LHS All Runs plot to {plot_path1}")
    print(f"Saved LHS Highlights plot to {plot_path2}")
    print(f"Saved LHS Heatmap distribution to {plot_path3}")
    ...
    if args.export_jl:
        ...
        with open(args.export_jl, "w") as f:
            ...
            f.write(f"# Top LHS Score: {all_final_energies[best_idx]:.3f}\n")
            f.write("wells = [\n")
            for w in range(num_wells):
    ...
    csv_path = f"plots/well_trajectories_batch_{timestamp}_results.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "run_id", "well_id", "well_type", "depth_z", 
            "final_x", "final_y", "mean_total_energy"
        ])
        for n_idx in range(num_samples_N):
            history = all_trajectories[n_idx]
            ...
            for w in range(num_wells):
                ...
                writer.writerow([
                    n_idx, w, w_type, z, x, y, mean_energy
                ])
