"""Full Pipeline Still — Scene Master Style.

An independent render showing the full pipeline:
Line 1: Well Pair -> Physics Slab -> 3D CNN -> MLP -> Edge Feature
Line 2: Well Graph -> Heterogeneous GNN -> Message Passing Graph -> Global Readout -> FNN -> Total Energy Production

Stylistically matches the last scene of scene_master (flat grey rectangles
...
class FullPipelineStill(Scene):
    def construct(self):
        ...
        # Draw arrows for Row 1
        arrows1 = VGroup()
        for i in range(len(row1) - 1):
            arr = Arrow(
                start=row1[i].get_right(),
                end=row1[i + 1].get_left(),
                **arr_kwargs
            )
            arrows1.add(arr)

        # Draw arrows for Row 2
        arrows2 = VGroup()
        for i in range(len(row2) - 1):
            arr = Arrow(
                start=row2[i].get_right(),
                end=row2[i + 1].get_left(),
                **arr_kwargs
            )
            arrows2.add(arr)

        # Connecting arrow from Edge Feature to Well Graph
        # We draw an orthogonal connecting line to avoid overlapping row1
        ...
        l1 = Line(pt1, pt2, color=WHITE, stroke_width=4)
        l2 = Line(pt2, pt3, color=WHITE, stroke_width=4)
        a3 = Arrow(pt3, pt4, color=WHITE, stroke_width=4, buff=0.0, max_tip_length_to_length_ratio=0.3)
